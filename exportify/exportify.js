...
const utils = {
  ...
  async authorize() {
    // This is bound to the login button in the HTML and gets called when the login button is clicked.
    let alphanumeric =
      "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789";
    let code_verifier = crypto
      .getRandomValues(new Uint8Array(64))
      .reduce((acc, x) => acc + alphanumeric[x % alphanumeric.length], "");
    let hashed = await crypto.subtle.digest(
      "SHA-256",
      new TextEncoder().encode(code_verifier)
    ); // some crypto methods are async
    let code_challenge = btoa(String.fromCharCode(...new Uint8Array(hashed)))
      .replace(/=/g, "")
      .replace(/\+/g, "-")
      .replace(/\//g, "_");
    ...
    location =
      "https://accounts.spotify.com/authorize?client_id=d99b082b01d74d61a100c9a0e056380b" +
      "&redirect_uri=" +
  ...
  async apiCall(url, delay = 0) {
    ...
    if (response.ok) {
      ...
      if (!error.innerHTML.includes("fa-bolt")) {
        error.innerHTML +=
          '<p><i class="fa fa-bolt" style="font-size: 50px; margin-bottom: 20px">\
				...
				rate limiting</a> while querying endpoint ' +
          url.split("?")[0] +
          '!<br/>Don\'t worry: Automatic backoff is implemented, and your data is \
...
class PlaylistTable extends React.Component {
  ...
  constructor(props) {
    super(props);
  } //render() gets called at the end of constructor execution
  ...
  async init() {
    ...
    let library = await utils.apiCall(
    ...
    let response = await utils.apiCall(
    ...
    await Promise.all(requests).then((responses) =>
      responses.map((response) => playlists.push(response.items))
    );
  ...
  sortRows(column) {
    ...
    allSorts.forEach((el) => {
      if (el != arrow) {
        el.className = "fa fa-fw fa-sort";
        el.style.color = "#C0C0C0";
      }
    }); // change the other two back to the greyed-out double-arrow
    if (
      arrow.className.endsWith("fa-sort") ||
      arrow.className.endsWith("fa-sort-asc")
    ) {
      arrow.className = "fa fa-fw fa-sort-desc";
    } //if the icon is fa-sort or asc, change to desc
    else if (arrow.className.endsWith("fa-sort-desc")) {
      arrow.className = "fa fa-fw fa-sort-asc";
    } //if descending, change to ascending
    arrow.style.color = "#000000"; // darken

    // rearrange table rows
    function field(p) {
      // get the keyed column contents
      if (column == "Name") {
        return p.name;
    ...
    this.setState({
      playlists: this.state.playlists.sort(
        (
          a,
          b // make sure to use setState() so React reacts! Calling render() doesn't cut the mustard.
        ) =>
          arrow.className.endsWith("desc") // figure out whether we're ascending or descending
            ? column == "Tracks"
              ? a.tracks.total - b.tracks.total
              : field(a).localeCompare(field(b)) // for numeric column, just use the difference to get a + or - number
            : column == "Tracks"
    ...
    if (this.state?.playlists.length > 0) {
      return React.createElement(
        ...
        React.createElement(
          ...
          React.createElement(
            "thead",
            null, // have to explicitly pass null because the children come as *args _after_ the second argument
            React.createElement(
              "tr",
              null,
              ...
              React.createElement(
                ...
                React.createElement(
                  "button",
                  {
                    ...
                    id: "exportAll",
                    onClick: () =>
                      PlaylistExporter.exportAll(this.state.playlists),
                  },
                  React.createElement("i", {
                  ...
                  React.createElement("img", {
                    src:
                      playlist.images?.length > 0
                        ...
                        : "https://placehold.co/30?text=blank",
                ...
                React.createElement(
                  ...
                  React.createElement(
                    "button",
                    {
                      ...
                      id: "export" + i,
                      onClick: () =>
                        PlaylistExporter.export(this.state.playlists[i], i),
                    },
                    React.createElement("i", {
...
let PlaylistExporter = {
  ...
  async export(playlist, row) {
    document.getElementById("export" + row).innerHTML =
      '<i class="fa fa-circle-o-notch fa-spin"></i> Exporting'; // spinner on button
    try {
      ...
      // change back the export button's text
      document.getElementById("export" + row).innerHTML =
    ...
    for (let playlist of playlists) {
      try {
        ...
        while (zip.file(fileName + ".csv")) {
          fileName += "_";
        } // Add underscores if the file already exists so playlists with duplicate names don't overwrite each other.
  ...
  fileName(playlist) {
    return playlist.name
      .replace(/[^a-z0-9\- ]/gi, "")
      .replace(/ /gi, "_")
      .toLowerCase(); // /.../gi is a Perl-style modifier, g for global, meaning all matches replaced, i for case-insensitive
  },
  ...
  async csvData(playlist) {
    ...
    for (let offset = 0; offset < playlist.tracks.total; offset += increment) {
      requests.push(
        utils.apiCall(
          playlist.tracks.href + "?offset=" + offset + "&limit=" + increment,
          (offset / increment) * 100
        )
      ); // I'm spacing requests by 100ms regardless of increment.
    }
    ...
    let data_promise = Promise.all(requests).then((responses) => {
      ...
      return responses.map((response) => {
        ...
        return response.items.map((song) => {
          // apply to all songs in each response
    ...
    let genre_promise = data_promise.then(() => {
      ...
      ); // volley of traffic, requests staggered by 100ms
      return Promise.all(artists_promises).then((responses) => {
        let artist_genres = {}; // build a dictionary, rather than a table
        responses.forEach((response) =>
          response.artists.forEach((artist) => {
            if (artist) {
              artist_genres[artist.id] = artist.genres.join(",");
            }
          })
        ); // these are the artists who had ids before, but it's still possible they aren't in the genre database
    ...
    let album_promise = Promise.all([data_promise, genre_promise]).then(() => {
      ...
      let album_chunks = [];
      while (album_ids.length) {
        album_chunks.push(album_ids.splice(0, 20));
      }
      let album_promises = album_chunks.map((chunk_ids, i) =>
        utils.apiCall(
          "https://api.spotify.com/v1/albums?ids=" + chunk_ids.join(","),
          120 * i
        )
      );
      return Promise.all(album_promises).then((responses) => {
        ...
        responses.forEach((response) =>
          response.albums.forEach((album) => {
            if (album) {
              record_labels[album.id] = album.label;
            }
          })
        );
    ...
    let features_promise = Promise.all([
      data_promise,
      genre_promise,
      album_promise,
    ]).then((values) => {
    ...
    return Promise.all([
      ...
      data.forEach((row) => {
        ...
        let deduplicated_genres = new Set(
          artist_ids
...
onload = async () => {
  ...
  if (
    ...
    Date.now() - localStorage.getItem("access_token_timestamp") < 3600000
  ) {
    loginButton.style.display = "none"; // When logged in, make the login button invisible
    logoutContainer.innerHTML =
      '<button id="logoutButton" class="btn btn-sm" onclick="utils.logout()">Log Out</button>'; // Add a logout button by modifying the HTML
